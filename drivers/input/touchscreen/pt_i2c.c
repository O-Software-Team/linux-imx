...
static int pt_i2c_read_default_(struct device *dev, void *buf, int size)
{
	...
	if (!buf || !size || size > I2C_MAX_BUF_SIZE)
		return -EINVAL;
...
/*******************************************************************************
 * FUNCTION: pt_i2c_read_default_nosize_
 *
...
/*******************************************************************************
 * FUNCTION: pt_i2c_write_read_specific_
 *
 * SUMMARY: Write the contents of write_buf to the I2C device and then read
 *  the response using pt_i2c_read_default_nosize_()
 *
 * PARAMETERS:
 *  *dev       - pointer to Device structure
