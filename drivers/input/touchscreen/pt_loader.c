...
/*******************************************************************************
 * FUNCTION: _pt_update_write_file_status
 *
 ...
 *  *update_status - pointer to store file update status
 ******************************************************************************/
static int _pt_update_write_file_status(struct device *dev, int *update_status,
				       u16 value)
{
	u8 inc = 0;
	int rc = 0;
	int temp_update_status;

	if (!update_status) {
		pt_debug(dev, DL_ERROR, "%s: update_status is not defined\n",
			 __func__);
		rc = -ENOMEM;
		return rc;
	}
	...
	pt_debug(dev, DL_DEBUG,
		...
		__func__, temp_update_status, value);

	/* Reset status if value is 0 */
	if (value == UPDATE_FW_IDLE) {
		temp_update_status = value;
	...
	if (value > UPDATE_FW_IDLE && value < UPDATE_FW_UNDEFINED_ERROR) {
		if (value <= UPDATE_FW_COMPLETE && value > temp_update_status) {
			temp_update_status = value;
	...
	if (value > UPDATE_FW_INCREMENT && value < UPDATE_FW_UNDEFINED_INC) {
		...
		if (temp_update_status + inc <= UPDATE_FW_COMPLETE)
			temp_update_status += inc;
		else
			pt_debug(dev, DL_ERROR,
				"%s: Inc Request out of bounds: status=%d inc=%d\n",
				__func__, temp_update_status, inc);
	} else
		pt_debug(dev, DL_ERROR,
...
exit:
	*update_status = temp_update_status;
	return rc;
}
		...
		int fw_size, int *update_status)
{
	...
	_pt_update_write_file_status(dev, update_status, UPDATE_FW_IDLE);
	...
	*update_status += 1;
	...
	if (!rc && cd->dual_mcu_available) {
		pt_debug(cd->dev, DL_INFO,
			"%s: Dual MCU is enabled, no enumeration after FW upgrade\n",
			__func__);
...
static int _pt_pip1_bl_from_file(struct device *dev)
{
	...
	pt_upgrade_firmware(dev, &(fw_img[header_size + 1]),
		fw_size - (header_size + 1), &ld->update_fw_status);
exit:
	kfree(fw_img);
	return rc;
}
...
static void _pt_firmware_cont(const struct firmware *fw, void *context)
{
	...
	pt_upgrade_firmware(dev, &(fw->data[header_size + 1]),
		fw->size - (header_size + 1), &ld->update_fw_status);
			      ...
			      u8 file_no, int *update_status)
{
	...
	if (ret < 0) {
		pt_debug(dev, DL_ERROR, "%s Open file %d failed\n",
			__func__, file_no);
		_pt_update_write_file_status(dev, update_status,
					     UPDATE_FW_FILE_OPEN_ERROR);
		goto exit;
	}
	file_handle = ret;
			      ...
			      u8 file_no)
{
	...
	u8 status = 0;
	u8 read_buf[PT_MAX_PIP2_MSG_SIZE];
	u16 saved_timeout = cd->pip_cmd_timeout;

	/*
	 * This would call resume function sychoniously and must be out of
	...
	ret = pt_pip2_write_file(dev, fw, file_no, &ld->update_fw_status);
	if (ret) {
	...
	if ((file_no == PIP2_RAM_FILE) &&
	    (ld->update_fw_status < UPDATE_FW_COMPLETE)) {
	...
	if (ld->update_fw_status > UPDATE_FW_COMPLETE)
		goto exit;
	...
	if (cd->enum_status & ENUM_STATUS_FW_RESET_SENTINEL) {
		/*
		 * We don't yet know the protocol of the new FW:
		 * Try PIP3 first
		 */
		...
		if (ret) {
			/* If PIP3(HID) failed, try PIP2 */
			pt_debug(dev, DL_INFO,
				"%s: PIP3 STATUS failed, try PIP2\n",
				__func__);
			mutex_lock(&cd->system_lock);
			...
			ret = cmd->request_pip2_get_mode_sysmode(dev,
				PT_CORE_CMD_UNPROTECTED, &mode, NULL);
			if (ret) {
				/* PIP3 and PIP2 failed */
				mutex_lock(&cd->system_lock);
				cd->protocol_mode = PT_MODE_UNKNOWN;
				mutex_unlock(&cd->system_lock);
			}
		}
		cd->pip_cmd_timeout_default = saved_timeout;
...
exit:
	 ...
	 */
	if (ld->pip2_load_builtin) {
		if ((ld->update_fw_status == UPDATE_FW_ACTIVE_99) ||
		    (ld->update_fw_status == UPDATE_FW_VERSION_ERROR))
			ld->builtin_bin_fw_status = 0;
		else
			ld->builtin_bin_fw_status = -EINVAL;
	}
...
static void _pt_pip2_write_file_cont(const struct firmware *fw, void *context)
{
	...
	if (!fw) {
		...
		_pt_update_write_file_status(dev, &ld->write_file_status,
					     UPDATE_FW_NO_FW_PROVIDED);
		goto exit;
	}
	...
	/* Write File Status: set to 100 */
	if (ld->write_file_status < UPDATE_FW_COMPLETE)
		_pt_update_write_file_status(dev, &ld->write_file_status,
					     UPDATE_FW_COMPLETE);
...
exit:
	...
	/*
	 * Temporary code: PtMfg can use "bl_status" to update pip2 file
	 * write status.
	 */
	ld->update_fw_status = ld->write_file_status;
}
 ...
 *
 * SUMMARY: Bootload the DUT with with firmware binary image in user space.
 *
 * RETURN:
 *   0 = success
 *  !0 = failure
...
static int _pt_pip2_update_fw_from_class(struct device *dev)
{
	 ...
	 */
	ld->pip2_load_builtin = false;
	pt_debug(dev, DL_INFO, "%s: Request FW Class\n", __func__);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* Ensure there is no running updater */
		...
		char *buf, loff_t offset, size_t count)
{
	...
	if (offset == 0) {
		...
		rc = cmd->nonhid_cmd->pip2_file_open(dev,
				ld->pip2_file_data.file_no);
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
...
exit_file_close:
	...
	if (rc)
		pt_debug(dev, DL_ERROR,
			"%s Failed to close handle %d, rc = %d\n", __func__,
			file_handle, rc);
	/*
...
exit_for_next_read:
	pt_debug(dev, DL_INFO,
		...
		"file_no", ld->pip2_file_data.file_no,
		...
		char *buf, loff_t offset, size_t count)
{
	...
	switch (length) {
	case 1:
		ld->pip2_file_data.file_no = ic_buffer[0];
		...
		struct device_attribute *attr, char *buf)
{
	...
	rc = cmd->nonhid_cmd->pip2_file_open(dev, file_no);
	if (rc < 0) {
		pt_debug(dev, DL_ERROR,
			"%s: Failed to file_open rc=%d\n",
			__func__, rc);
	...
	if (ld->pip2_fcrc.file_read_size == -1)
		length = file_size - offset;

	if (length > file_size || offset > file_size
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ld->pip2_fcrc.file_no = PIP2_RAM_FILE;
	...
	if (length == 1) {
		ld->pip2_fcrc.file_no = input_data[0];
		ld->pip2_fcrc.file_offset = 0;
		ld->pip2_fcrc.file_read_size = -1;
		ld->pip2_fcrc.para_num = 1;
		pt_debug(dev, DL_INFO, "%s: %s=%d, %s=%d\n",
			__func__,
		...
		struct device_attribute *attr, char *buf)
{
	...
	rc1 = cmd->request_exclusive(dev, PT_LDR_REQUEST_EXCLUSIVE_TIMEOUT);
	if (rc1 < 0) {
		pt_debug(dev, DL_ERROR,
	...
	rc1 = cmd->nonhid_cmd->pip2_file_open(dev, file);
	if (rc1 < 0) {
		rc1 = -EBADF;
		goto exit_release;
	}
	...
	rc1 = cmd->nonhid_cmd->pip2_file_get_stats(dev,
			file_handle, NULL, &max_file_size);
	if (rc1) {
		pt_debug(dev, DL_ERROR,
			"%s: Failed to get_file_state ret=%d\n",
			__func__, rc1);
	...
	rc1 = cmd->nonhid_cmd->pip2_file_erase(dev, file_handle,
		sector_num, &pip2_erase_status);
	if (rc1) {
		pt_debug(dev, DL_INFO, "%s: File erase error rc = %d\n",
			__func__, rc1);
	} else {
		pt_debug(dev, DL_INFO, "%s: handle %d erased\n",
			__func__, file_handle);
	}

exit_file_close:
	rc2 = cmd->nonhid_cmd->pip2_file_close(dev, file_handle);
	if (rc2)
		pt_debug(dev, DL_INFO, "%s: File close error rc = %d\n",
			__func__, rc2);
...
/*******************************************************************************
 * FUNCTION: pt_fw_status_
 *
 * SUMMARY: Function to store the percent completion of the current BL or file
 *  write or an error message.
 *
 * RETURN:
 *   Length of data written to buf
 *
 * PARAMETERS:
 *	*dev   - pointer to device structure
 *	*buf   - pointer to print output buffer
 *	status - fw upgrade status or file write status
 ******************************************************************************/
static ssize_t pt_fw_status_(struct device *dev, char *buf, int status)
{
	ssize_t ret;

	if (status <= UPDATE_FW_COMPLETE) {
		pt_debug(dev, DL_DEBUG,
			"%s BL_STATUS = %d\n", __func__, status);
	...
	case UPDATE_FW_FILE_SEEK_ERROR:
		ret = sprintf(buf, "ERROR: %d - File seek failure\n", status);
		break;
	...
	case UPDATE_FW_PROGRAM_ROW_ERROR:
		ret = sprintf(buf,
			"ERROR: %d - Failed to program FW image\n", status);
		break;
 ...
 *
 * SUMMARY: The show method for the pip2_bl_status and update_fw_status sysfs
 *  nodes. Shows the percent completion of the current BL or an error message.
 *
 * PARAMETERS:
 *	*dev  - pointer to device structure
 *	*attr - pointer to device attributes structure
 *	*buf  - pointer to print output buffer
 ******************************************************************************/
static ssize_t pt_update_fw_status_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_loader_data *ld = pt_get_loader_data(dev);
	ssize_t ret;
	u8 status = ld->update_fw_status;

	ret = pt_fw_status_(dev, buf, status);
 ...
 *
 * SUMMARY: The show method for the pip2_file_write_status sysfs node.
 *	Shows the percent completion of file write or an error message.
 *
 * PARAMETERS:
 *	*dev  - pointer to device structure
 *	*attr - pointer to device attributes structure
		...
		struct device_attribute *attr, char *buf)
{
	struct pt_loader_data *ld = pt_get_loader_data(dev);
	ssize_t ret;
	u8 status = ld->write_file_status;
...
static DEVICE_ATTR(pip2_bl_status, 0444, pt_update_fw_status_show, NULL);
static DEVICE_ATTR(pip2_file_write_status, 0444,
	pt_write_file_status_show, NULL);
#if PT_FW_UPGRADE
static DEVICE_ATTR(update_fw_status, 0444, pt_update_fw_status_show, NULL);
#endif
/*******************************************************************************
...
#ifdef CONFIG_TOUCHSCREEN_PARADE_PLATFORM_FW_UPGRADE
	if (dut_gen == DUT_PIP1_ONLY) {
		pt_debug(dev, DL_WARN, "%s: Upgrade Platform FW\n", __func__);
		if (!upgrade_firmware_from_platform(dev, false))
			return 0;
	}
#endif
...
#ifdef CONFIG_TOUCHSCREEN_PARADE_BINARY_FW_UPGRADE
	pt_debug(dev, DL_WARN, "%s: Upgrade Builtin FW\n", __func__);
	if (dut_gen == DUT_PIP2_CAPABLE) {
		if (!pt_pip2_upgrade_firmware_from_builtin(dev))
			return 0;
		pt_debug(dev, DL_WARN, "%s: Builtin FW upgrade failed\n",
...
static int pt_loader_probe(struct device *dev, void **data)
{
	...
	if (dut_gen == DUT_PIP2_CAPABLE) {
		...
		rc = device_create_file(dev, &dev_attr_pip2_file_write_status);
		if (rc) {
			pt_debug(dev, DL_ERROR,
				"%s: Error creating pip2_file_write_status\n",
				__func__);
			goto remove_files;
		}
...
remove_files:
	...
	device_remove_file(dev, &dev_attr_pip2_file_write_status);
