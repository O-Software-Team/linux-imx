 ...
 *
 * 1) Take noise test as reference, total test result=Rows*Columns*2*2 (bytes).
 *    The Rows*Columns is to calculate sensor numbers. The maximum supported
 *    sensor numbers is limited by MAX_SENSORS while it can't be
 *    MAX_TX_SENSORS*MAX_RX_SENSORS. So the calculation is MAX_SENSORS*4.
 * 2) Format PT_PR_FORMAT_U8_SPACE consumes more print buffer than others:
 *    Each byte to print needs 2 character and 1 space.
 * 3) Usual prefix and status value print is about 70 bytes, and considering
 *    64*n as a way to allocate buffer and decide 128 to ensure safe.
 */
#define PR_BUFFER_MAX_SIZE      (MAX_SENSORS * 4 * 3 + 128)
	...
	struct cmcp_data *cmcp_info)
{
	...
	pt_debug(dev, DL_INFO, "%s tx_num=%d\n", __func__, tx_num);
	pt_debug(dev, DL_INFO, "%s rx_num=%d\n", __func__, rx_num);
	pt_debug(dev, DL_INFO, "%s btn_num=%d\n", __func__, btn_num);
	...
	struct test_case_field *field_array, struct configuration *config)
{
	...
	if (config->cm_enabled && config->cp_enabled) {
		...
		/* Invalid Gradient Column/Row threshold */
		col_num = field_array[CM_GRADIENT_CHECK_COL].data_num;
		if (config->col_num != col_num) {
			config->is_valid_or_not = 0;
			pt_debug(dev, DL_ERROR,
				"Col number mismatch, Expect=%d, gradient_col=%d\n",
				config->col_num, col_num);
		}
		row_num = field_array[CM_GRADIENT_CHECK_ROW].data_num;
		if (config->row_num != row_num) {
			config->is_valid_or_not = 0;
			pt_debug(dev, DL_ERROR,
				"Row number mismatch, Expect=%d, gradient_row=%d\n",
				config->row_num, row_num);
		}

		/* Invalid CM Delta Column/Row threshold */
		threshold_exist =
