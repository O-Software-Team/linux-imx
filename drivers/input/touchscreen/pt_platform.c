 ...
 *  level.
 *
 * NOTE: The funtion only contains setup for VCC and VDDI since AVDD AVEE is
			     ...
			     struct device *dev)
{
	...
	if (IS_ERR(pdata->vcc)) {
		...
		pt_debug(dev, DL_ERROR, "get vcc regulator failed,rc=%d\n", rc);
		return rc;
	}
	...
	if (IS_ERR(pdata->vddi)) {
		...
		pt_debug(dev, DL_ERROR,
			"get vddi regulator failed,rc=%d\n", rc);
		goto error_get_vcc;
	}
	...
	if (regulator_count_voltages(pdata->vddi) > 0) {
		...
		if (rc) {
			pt_debug(dev, DL_ERROR,
				 "set vddi regulator failed rc=%d\n", rc);
			goto error_set_vddi;
		}
	}
 ...
 *  *dev   - pointer to Device structure
 *  *time  - pointer to store how many ms this function will sleep. This is
 *           used to calculate how long to leave XRES asserted during pwr up
 ******************************************************************************/
static int pt_setup_power_by_regulator(struct pt_core_platform_data *pdata,
				       int on, struct device *dev, int *time)
{
	int rc = 0;
	...
	if (on == PT_MT_POWER_ON) {
		...
		if (rc) {
			pt_debug(dev, DL_ERROR,
				 "enable vddi regulator failed,rc=%d\n", rc);
		}

		rc = regulator_enable(pdata->vcc);
		if (rc) {
			pt_debug(dev, DL_ERROR,
				 "enable vcc regulator failed,rc=%d\n", rc);
		}
		/*
		...
		*time = 0;
		...
		if (rc) {
			pt_debug(dev, DL_ERROR,
				 "disable vcc regulator failed,rc=%d\n", rc);
		}
		rc = regulator_disable(pdata->vddi);
		if (rc) {
			pt_debug(dev, DL_ERROR,
				 "disable vddi regulator failed,rc=%d\n", rc);
		}
		...
		int on, struct device *dev)
{
	...
	if (on && rst_gpio) {
		...
		if (rc < 0) {
			...
			/*
			 * The RST GPIO is typically an output but during init,
			 * before power is applied, set it up as an input to
			 * allow it to float up with VDDI when power is applied.
			 */
				  ...
				  int on, struct device *dev, int *time)
{
	...
	if (on == PT_MT_POWER_ON) {
		...
		/*
		 * 2-3ms after enabling VDDI and VCC, change the direction of
		...
		/* Reset ms counter since XRES was enabled */
		*time = 0;
		...
		struct device *dev)
{
	 ...
	 * - For Gen6 the RST line must be disabled (set high) when VDDI is
	 * being applied for a min of 2ms, after which it must be enabled (set
	 * low) for 10ms.
	 * - For TT/TC parts it must be low while power is applied but the pwr
	 * sequence for Gen6 is ok.
	 */
	if (on && pdata->rst_gpio) {
		/*
		...
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
				...
				__func__, pdata->rst_gpio);
			/*
			 * Not ideal but if setting the direction to input
			 * failed ensure part is not in reset during first 2ms.
			 */
...
#ifdef PT_REGULATOR_EN
	rc = pt_setup_power_by_regulator(pdata, on, dev, &sleep_time);
	if (rc) {
		pt_debug(dev, DL_ERROR,
	...
	rc = pt_setup_power_by_gpio(pdata, on, dev, &sleep_time);
	if (rc) {
		pt_debug(dev, DL_ERROR,
	...
	/*
	 * TP_XRES must be held low for a min of 10ms, however, TDDI panels
	 * will have already slept for 8ms so only add 2ms, whereas TT7K/Gen6
