...
static int pt_spi_xfer(struct device *dev, u8 op, u8 *buf, int length)
{
	 ...
	 * TT/TC parts include an extra prefix byte so the data starts at
	 * index 2 as compared to index 1 for Gen6.
	 */
	if (op == PT_SPI_RD_OP) {
		if (cd->active_dut_generation == DUT_PIP1_ONLY)
			memcpy(buf, &tmp_rbuf[1], length);
		else
...
static void pt_spi_remove(struct spi_device *spi)
#else
