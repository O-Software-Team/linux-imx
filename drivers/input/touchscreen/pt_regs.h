...
enum PT_STARTUP_STATUS {
	ENUM_STATUS_START              = 0,
	ENUM_STATUS_BL_RESET_SENTINEL  = 0x001,
	ENUM_STATUS_FW_RESET_SENTINEL  = 0x002,
	ENUM_STATUS_GET_DESC           = 0x004,
	ENUM_STATUS_FW_OUT_OF_BOOT     = 0x008,
	ENUM_STATUS_GET_RPT_DESC       = 0x010,
	ENUM_STATUS_GET_SYS_INFO       = 0x020,
	ENUM_STATUS_GET_CFG_CRC        = 0x040,
	ENUM_STATUS_RESTORE_PARM       = 0x080,
	ENUM_STATUS_COMPLETE           = 0x100,
	ENUM_STATUS_FULL               = 0x1FF
};
...
enum PT_HID_REPORT_ID {
	...
	PT_HID_OUTPUT_REPORT_ID            = 0x04,
	PT_HID_VS_FINGER_REPORT_ID         = 0x41, /* Vendor Specific ID */
	PT_HID_VS_PEN_REPORT_ID            = 0x42, /* Vendor Specific ID */
	PT_HID_WRAP_PIP_REPORT_ID          = 0x44, /* Vendor Specific ID */
	/*
	 * The asynchronous reports generated by SENSOR_DATA_MODE,
	 * TRACKING_HEATMAP and REALTIME_SIGNAL_DATA_MODE will
	 * use report ID 0x45.
	 */
	PT_HID_ASYNC_SENSOR_DATA_REPORT_ID = 0x45 /* Vendor Specific ID */
};
...
#define HID_APP_REPORT_DESC_ID                  0xF6
#define HID_BL_REPORT_DESC_ID                   0xFE
...
#define HID_SYSINFO_BTN_OFFSET                    49
...
/* FUTURE: Need measure a case and then double it */
#define PT_HID_MULTIPACKET_TIMEOUT             30000
...
#define IS_PIP_VER_EQ(p, maj, min) \
		((p)->ttdata.pip_ver_major == (maj) ? \
			...
			1 : \
				0 : \
				0))
#define IS_BL_SENTINEL(p, size) \
		(size == 11 && \
			((p)->input_buf[PIP2_RESP_SEQUENCE_OFFSET] & \
			PIP2_RESP_SEQUENCE_MASK) == 0 && \
			((p)->input_buf[PIP2_RESP_REPORT_ID_OFFSET] & \
			PIP2_CMD_COMMAND_ID_MASK) == \
			PIP2_CMD_ID_STATUS && \
			(p)->input_buf[PIP2_RESP_STATUS_OFFSET] == 0)
...
#define PT_DRV_DBG_CORE_SET_PROTOCOL_MODE    221
...
#define PT_PIP2_CMD_REGISTER (0x0101)
...
enum PIP2_CMD_ID {
	...
	PIP2_CMD_REALTIME_SIGNAL_MODE   = 0x44,
...
#define HID_CMD_PAYLOAD_LEN_OFFSET         (9)
#define HID_CMD_COMMAND_ID_OFFSET          (12)

/* HID response offset/size */
#define HID_RESP_REPORT_ID_OFFSET          (2)
#define HID_RESP_HEADER_SIZE               (4)
#define HID_RESP_MRPT_OFFSET               (3)
#define HID_RESP_PAYLOAD_LEN_OFFSET        (4)
...
#define HID_PAYLOAD_RESP_ARL_OFFSET        (5)
#define HID_PAYLOAD_RESP_DATA_OFFSET       (7)

/*
 * Calculate the memory to store whole packets for PIP embedded HID response.
 *
 * PIP1 maximum RESP size change(PT_MAX_PIP1_MSG_SIZE+1) when embedded in HID:
 ...
 *                                 2 byte CRC
 * PIP2 maximum RESP size has not change when embedded in HID:
 *   < PIP2 >                      < PIP2 Embedded in HID >
 *   2 byte Lengh field            2 byte Lengh field
 *   1 byte SEQ                    1 byte SEQ
 *   1 byte CMD ID             =>  1 byte CMD ID
 *   N byte Report Body            N byte Report Body
 *   2 byte CRC                    2 byte CRC2 byte CRC
 * Above information shows the size of PIP payload and PIP1/PIP2 response
 * (very close). Since HID layer will deal with large response rather than PIP
 * protocol, the PAYLOAD can be over 255 bytes. The largest payload is
 * GetSelfTestResults comand, The maximum supported sensors are calculated as
 *    171(Channels) * 8(chips) * 6(Mux) = 8208;
 * then HID_MAX_PAYLOAD_SIZE (e.g.Noise test) is:
 *    HID_MAX_PAYLOAD_SIZE = 2*2*8208 + 9(bytes header) = 32841 bytes.
 *    (This should not be reached since layout limit)
 * The usual hid_max_input_len is 37. So the calculation is about:
 *    HID_MAX_PAYLOAD_SIZE/(37-HID_RESP_HEADER_SIZE)
 * And rounded up as:
 *    (HID_MAX_PAYLOAD_SIZE-1)/(37-HID_WRAP_PIP_HEADER_SIZE) + 1
 * The MAX_HID_WRAPPER_PACKETS is calculated as 1095.
 *
 * The maximum used memory is calculated as:
 *  MAX_HID_WRAPPER_PACKETS * HID_WRAP_PIP_HEADER_SIZE + (maximum PIP response)
 * The calculation is 40506.
 * Then 40960 is suggested for buffer to store these HID reports.
 */
...
struct PIP_vs_PURE_HID_cmd_id_map {
	u8 pip_cmd_id;
	u8 hid_cmd_id;
};
...
struct pt_hid_report_buf_op {
	u8 buf[HID_BUF_OP_BUFFER_SIZE];
	u16 offset;
	enum pt_report_type report_type;
	u16 last_remain_packet;
};

struct pt_hid_async_report {
	u8 mrpt;
	u8 frpt;
	u8 cmd_id;
...
enum pt_protocol_mode {
	...
	PT_PROTOCOL_MODE_HYBRID_HID   = 1,
	PT_PROTOCOL_MODE_HID          = 2,
...
enum PT_ENTER_BL_RESULT {
	...
	PT_ENTER_BL_START_BL_CMD_FAIL   = 3,
...
struct pt_tch_abs_params {
	...
	size_t logical_min;	/* logical min value */
};
...
/*
 * FUTURE: Align with HID buffer as temporary solution, in future it should have
 * framework level improvement
 */
#define PT_MAX_INPUT            HID_BUF_OP_BUFFER_SIZE
...
struct pt_mt_function {
	...
	void (*report_touch_liftoff)(struct pt_mt_data *md, int t);
};
...
#define PT_PREALLOCATED_CMD_BUFFER 32
	...
	unsigned int int_period_min;
	unsigned int int_period_max;
	u16 err_gpio;
	u16 err_gpio_type;
	bool show_tt_data;
	bool bridge_mode;
...
#ifdef CONFIG_TOUCHSCREEN_PARADE_PEN
int pt_pen_probe(struct device *dev);
int pt_pen_release(struct device *dev);
#else
static inline int pt_pen_probe(struct device *dev) { return 0; }
static inline int pt_pen_release(struct device *dev) { return 0; }
#endif
...
static inline unsigned int pt_get_time_stamp_us(void)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 0, 0))
	struct timespec64 ts;
