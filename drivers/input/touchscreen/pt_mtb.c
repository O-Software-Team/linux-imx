...
#ifdef PT_SEND_LIFTOFF_EARLY
/*******************************************************************************
 * FUNCTION: pt_report_touch_liftoff
 *
 * SUMMARY: Function to report single touch is lifted
 * protocol
 *
 * PARAMETERS:
 *  *md - pointer to input device structure
 *  t   - the touch id of the lifted touch
 ******************************************************************************/
static void pt_report_touch_liftoff(struct pt_mt_data *md, int t)
{
	input_mt_slot(md->input, t);
	input_mt_report_slot_state(md->input,
		MT_TOOL_FINGER, false);
...
#else
#define pt_report_touch_liftoff      NULL
#endif
...
void pt_init_function_ptrs(struct pt_mt_data *md)
{
	...
	md->mt_function.report_touch_liftoff = pt_report_touch_liftoff;
}
