		...
		struct pt_hid_cmd *hid_cmd)
{
	...
	pt_debug(cd->dev, DL_INFO,
		...
		__func__, cmd_length, cmd[HID_CMD_COMMAND_ID_OFFSET]);
 ...
 *
 * SUMMARY: Wrapper function to send PIP3 command to DUT'S PIP3 command
 *	 register.
 *
 * RETURN:
 *	 0 = success
 ...
 * PARAMETERS:
 *  *cd         - pointer to core data
 *  *pip3_cmd   - pointer to PIP3 command data structure
 ******************************************************************************/
static int pt_pip3_send_cmd_(struct pt_core_data *cd,
	struct pt_pip3_cmd *pip3_cmd)
{
	int rc = 0;
	u16 index = 0;
	 ...
	 *   CRC(2)
	 */
	payload_len = pip3_cmd->param_len + 6;
	hid_cmd.write_buf[index++] = LOW_BYTE(payload_len);
	hid_cmd.write_buf[index++] = HI_BYTE(payload_len);

	/* Request and set SEQ(1) */
	pip3_cmd->cmd_tag_seq = pt_pip2_get_next_cmd_seq(cd);
	hid_cmd.write_buf[index++] = pip3_cmd->cmd_tag_seq;

	/* Set command ID(1) */
	hid_cmd.write_buf[index++] = pip3_cmd->cmd_id;

	/* Copy PIP3 report body(N) */
	memcpy(&hid_cmd.write_buf[index], pip3_cmd->param,
	       pip3_cmd->param_len);
	index += pip3_cmd->param_len;

	/*
	 * Calcualte CRC(2): from 'Length of payload' to 'PIP3 report body'
	 */
	crc = crc_ccitt_calculate(&hid_cmd.write_buf[3],
		index - 3);
	hid_cmd.write_buf[index++] = (crc & 0xff00) >> 8;
	hid_cmd.write_buf[index++] = (crc & 0xff);
...
/*******************************************************************************
 * FUNCTION: pt_pip3_send_and_wait_
 *
 * SUMMARY: Send valid PIP3 command to the DUT and wait for the response.
 *
 * RETURN:
 *	 0 = success
 *	!0 = failure
 ...
 *  *pip3_cmd  - pointer to PIP3 command data structure
 ******************************************************************************/
static int pt_pip3_send_and_wait_(struct pt_core_data *cd,
		struct pt_pip3_cmd *pip3_cmd)
{
	int rc = 0;
	int t;
	...
	mutex_lock(&cd->system_lock);
	cd->hid_cmd_state = pip3_cmd->cmd_id + 1;
	cd->report_type = PIP3_CMD_REPORT;
	mutex_unlock(&cd->system_lock);

	if (pip3_cmd->timeout_ms)
		timeout_ms = pip3_cmd->timeout_ms;
	else
		timeout_ms = cd->pip_cmd_timeout_default;

	rc = pt_pip3_send_cmd_(cd, pip3_cmd);
	if (rc)
		goto error;
again:
	t = wait_event_timeout(cd->wait_q, (cd->hid_cmd_state == 0),
...
#endif /* TTDL_DIAGNOSTICS */
		pt_debug(cd->dev, DL_ERROR,
			"%s: PIP3 cmd execution timed out (%dms)\n",
			__func__, timeout_ms);
		rc = -ETIME;
		goto error;
	}
...
/*******************************************************************************
 * FUNCTION: pt_pip_ver_load_ttdata
 *
 * SUMMARY: Function to load the Version information from the PIP2/PIP3 VERSION
 ...
 ******************************************************************************/
static void pt_pip_ver_load_ttdata(struct pt_core_data *cd, u16 len)
{
...
static int pt_pip2_get_version_(struct pt_core_data *cd)
{
	...
	if (status == 0) {
		...
		pt_pip_ver_load_ttdata(cd, actual_read_len);
	} else {
		pt_debug(cd->dev, DL_ERROR,
...
/*******************************************************************************
 * FUNCTION: pt_pip3_get_version_
 *
 * SUMMARY: Send PIP3 VERSION command 0x07 to the DUT and stores the data in
 *	cd-ttdata
 *
 ...
 *	*cd       - pointer to core data
 ******************************************************************************/
int pt_pip3_get_version_(struct pt_core_data *cd)
{
	int rc = 0;
	int pl_len;
	struct pt_pip3_cmd pip3_cmd = {
		.cmd_id = PIP3_CMD_ID_VERSION,
		.timeout_ms = PT_HID_CMD_DEFAULT_TIMEOUT,
	};
	rc = pt_pip3_send_and_wait_(cd, &pip3_cmd);
	if (rc) {
		pt_debug(cd->dev, DL_ERROR,
			"%s: Error Sending PIP3 VERSION Cmd rc=%d\n",
			__func__, rc);
		return rc;
	}

	pl_len = get_unaligned_le16(&cd->response_buf[0]);

	/* Parse the PIP3 VERSION response into ttdata */
	pt_pip_ver_load_ttdata(cd, pl_len);
...
/*******************************************************************************
 * FUNCTION: pt_pip3_get_fw_cat_id_
 *
 * SUMMARY: Determine the FW category ID of the DUT by use of the PIP3 VERSION
 *	command.
 *
 * RETURN:
 *	 0 = success
 ...
 * PARAMETERS:
 *	*cd         - pointer to core data structure
 *	*fw_cat_id  - pointer to store the FW category ID
 ******************************************************************************/
static int pt_pip3_get_fw_cat_id_(struct pt_core_data *cd, u8 *fw_cat_id)
{
	int rc = 0;

	if (cd->mode != PT_MODE_OPERATIONAL) {
		pt_debug(cd->dev, DL_WARN,
			"%s: FW is not in operational mode, cannot get fw category ID\n",
			__func__);
		rc = -EINVAL;
		goto exit;
	}

	rc = pt_pip3_get_version_(cd);
	if (!rc && fw_cat_id) {
		*fw_cat_id = cd->sysinfo.ttdata.fw_category_id;
		pt_debug(cd->dev, DL_INFO, "%s: fw category ID: %d\n",
			__func__, *fw_cat_id);
	}
...
{
	int rc = 0;
	u8 temp_mode = PT_MODE_UNKNOWN;
	u8 temp_sys_mode = FW_SYS_MODE_UNDEFINED;

	rc = pt_pip2_get_status_(cd);
	if (!rc || (rc == PIP2_RSP_ERR_INIT_FAILURE)) {
		temp_sys_mode = cd->dut_status.fw_system_mode;
		temp_mode = cd->dut_status.mode;
	} else {
		pt_debug(cd->dev, DL_WARN,
	...
	int *actual_wait)
{
	...
	if (sys_mode == FW_SYS_MODE_BOOT) {
		...
		*actual_wait = (int)(loop * pause);
		if (rc == PIP2_RSP_ERR_INIT_FAILURE)
			pt_debug(cd->dev, DL_ERROR,
				"%s: Initialization error detected, FW unable to exit bootup mode. rc = %d\n",
				__func__, rc);
		else
			pt_debug(cd->dev, DL_WARN,
	...
	u8 *prot_mode)
{
	 ...
	 */
	/* Shorten timeout to speed up detection */
	mutex_lock(&cd->system_lock);
	save_timeout = cd->pip_cmd_timeout;
	...
	/* Restore timeout */
	mutex_lock(&cd->system_lock);
	cd->pip_cmd_timeout = save_timeout;
	mutex_unlock(&cd->system_lock);
	if (rc) {
		pt_debug(dev, DL_ERROR,
 ...
 *  protocol mode known as HID and mode could be Operational.
 *
	...
	u8 *prot_mode)
{
	...
	/*
	 * 001-30009*L:Since there is no PIP3 ROM bootloader, here
	 * assign below variables directly.
	 */
	dut_gen_tmp = DUT_PIP2_CAPABLE;
...
static int _pt_request_hw_version(struct device *dev, char *hw_version)
{
	...
	if (cd->active_dut_generation == DUT_PIP2_CAPABLE) {
		...
		if (status == 0) {
			 ...
			 * swapped. PIP3.3 and greater the version fields
			 * are moved back 4 bytes.
			 */
			if (pip_ver >= 0x0303) {
				snprintf(hw_version, HW_VERSION_LEN_MAX,
					"%02X%02X.%02X%02X.FF",
					rd_buf[index + 14], rd_buf[index + 13],
					rd_buf[index + 12], rd_buf[index + 11]);
			} else if (pip_ver >= 0x0202) {
				snprintf(hw_version, HW_VERSION_LEN_MAX,
					"%02X%02X.%02X%02X.FF",
				      ...
				      u16 *remain_packets, int *report_type)
{
	...
	if (frpt) {
		...
		/*
		 * Clear structure pt_hid_report_buf_op before
...
exit:
	/* Clear the offset of structure pt_hid_report_buf_op */
	if (mrpt == 0 || rc)
		cd->pt_hid_buf_op.offset = 0;
...
#ifdef TTHE_TUNER_SUPPORT
	if (cd->bridge_mode && size > 0)
		tthe_print(cd, cd->input_buf, size, "BMD=");
#endif
...
#ifdef TTHE_TUNER_SUPPORT
			if (cd->bridge_mode)
				tthe_print(cd, cd->input_buf, 2, "BMD=");
#endif
			mutex_lock(&cd->system_lock);
			cd->pip2_prot_active = false;
			if (cd->active_dut_generation == DUT_PIP1_ONLY) {
...
skip_tthe_print:
	...
	if (!cd->bridge_mode && touch_report)
...
irqreturn_t pt_irq(int irq, void *handle)
{
	...
	if (cd->t_refresh_active) {
		if (cd->t_refresh_count == 0) {
			...
			cd->t_refresh_count++;
			start_t = pt_get_time_stamp_us();
			cd->int_period_min = start_t;
			cd->int_period_max = 0;
		} else {
			if (cd->t_refresh_count < cd->t_refresh_total) {
				cd->t_refresh_count++;
				end_t = pt_get_time_stamp_us();
			...
			if (elapsed_t < cd->int_period_min)
				cd->int_period_min = elapsed_t;
			if (elapsed_t > cd->int_period_max)
				cd->int_period_max = elapsed_t;
		}
	}
#endif /* TTDL_DIAGNOSTICS */
...
reset:
	...
	case PT_MODE_OPERATIONAL:
		...
		if ((cd->app_pip_ver_ready == false) ||
			...
			if (!rc) {
				...
				cd->fw_category_id =
					cd->sysinfo.ttdata.fw_category_id;
				if (cd->fw_category_id == PT_PROGRAMMER_FW) {
					pt_debug(cd->dev, DL_INFO, "%s: FW category: Secondary\n",
						__func__);
				} else if (cd->fw_category_id
							== PT_UTILITY_FW) {
					pt_debug(cd->dev, DL_INFO, "%s: FW category: Utility\n",
						__func__);
				} else
					pt_debug(cd->dev, DL_INFO, "%s: FW category: Primary\n",
						__func__);
			} else {
				if (try++ < PT_CORE_STARTUP_RETRY_COUNT)
					goto reset;
	...
	*enum_status |= ENUM_STATUS_GET_RPT_DESC;
	...
	if (cd->fw_category_id == PT_PROGRAMMER_FW ||
		cd->fw_category_id == PT_UTILITY_FW) {
		pt_debug(cd->dev, DL_INFO,
...
_retry:
	...
	case PT_MODE_UNKNOWN:
		/*
		 * When the mode could not be determined due to the DUT
		 * potentially running corrupted FW or FW that is not
		 * responding to the mode request, a hard reset is done.
		 */
		break;
	case PT_MODE_OPERATIONAL:
		if (sys_mode == FW_SYS_MODE_SCANNING &&
			cd->fw_category_id == PT_TOUCH_FW) {
...
int _pt_pip2_file_open(struct device *dev, u8 file_no)
{
	...
	if (rc || ((status != PIP2_RSP_ERR_NONE) &&
		...
		pt_debug(dev, DL_INFO, "%s: File %d is opened, handle=%d\n",
			 __func__, file_no, file_handle);
	} else {
		return -status;
	}
...
int _pt_pip2_file_close(struct device *dev, u8 file_handle)
{
	...
	pt_debug(dev, DL_DEBUG, "%s: CLOSE handle %d\n", __func__, file_handle);
	data[0] = file_handle;
	...
	if (ret) {
		pt_debug(dev, DL_ERROR,
			"%s ROM BL FILE_CLOSE timeout for handle = %d\n",
			__func__, file_handle);
		return -ETIME;
	}
	...
	if (status != 0x00) {
		pt_debug(dev, DL_ERROR,
			"%s ROM BL FILE_CLOSE failure: 0x%02X for handle = %d\n",
				   ...
				   int *status)
{
	...
	pt_debug(dev, DL_DEBUG, "%s: ERASE handle %d\n", __func__, file_handle);
	data[0] = file_handle;
	...
	*status = PIP2_RSP_ERR_TIMEOUT;
...
#endif
			pt_debug(dev, DL_WARN,
				"%s: ERASE timeout %d for handle = %d\n",
	...
	if (ret) {
		pt_debug(dev, DL_ERROR,
			"%s ROM FILE_ERASE cmd failure: %d for handle = %d\n",
	...
	if (*status != 0x00) {
		pt_debug(dev, DL_ERROR,
			"%s ROM FILE_ERASE failure: 0x%02X for handle = %d\n",
			__func__, *status, file_handle);
		return -EIO;
	}
	return ret;
				       ...
				       u16 file_sector, int *status)
{
	...
	pt_debug(dev, DL_DEBUG, "%s: ERASE handle=%d, sector=%d\n", __func__,
...
#endif
			pt_debug(dev, DL_WARN,
			    "%s: ERASE timeout %d for handle=%d, sector=%d\n",
	...
	if (ret) {
		pt_debug(dev, DL_ERROR,
			"%s ROM FILE_ERASE cmd failure: %d for handle=%d, sector=%d\n",
	...
	if (*status != 0x00) {
		pt_debug(dev, DL_ERROR,
			"%s ROM FILE_ERASE failure: 0x%02X for handle=%d, sector=%d\n",
...
int _pt_request_pip2_bin_hdr(struct device *dev, struct pt_bin_file_hdr *hdr)
{
	...
	if (cd->flashless_dut) {
		...
		ret = _pt_pip2_file_open(dev, PIP2_FW_FILE);
		if (ret < 0) {
			ret = -ENOENT;
			pt_debug(dev, DL_ERROR,
				 "%s Failed to open bin file, ret=%d\n",
				 __func__, ret);
			goto exit;
		}
		file_handle = ret;
		ret = 0; /* Reset ret to 0(success) */
...
exit_close_file:
		/* Close the file */
		if (_pt_pip2_file_close(dev, file_handle)) {
			ret = -EX_ERR_FCLOSE;
			pt_debug(dev, DL_ERROR,
	...
	u32 *file_size)
{
	...
	if (ret || (status != 0x00)) {
		pt_debug(dev, DL_ERROR,
			"%s ROM FILE_STATS failure: 0x%02X for handle = %d, ret = %d\n",
	...
	u32 read_offset, u32 write_offset)
{
	...
	if (ret || (status != 0x00)) {
		pt_debug(dev, DL_ERROR,
			"%s ROM FILE_SEEK failure: 0x%02X for handle = %d, ret = %d\n",
			__func__, status, file_handle, ret);
		ret = -EIO;
	}
	return ret;
		...
		size_t count, loff_t *ppos)
{
	...
	if (list->boundary_hit) {
		list->boundary_hit = false;
		goto out;
	}
	...
	fifo_len = kfifo_len(&list->tuner_buf_fifo);
	if (fifo_len >= PT_MAX_PRBUF_SIZE * 3) {
		pt_debug(cd->dev, DL_WARN,
	...
	ret = copied;
	/*
	 * When hitting the boundary of user buffer, userspace method will read
	 * one more loop to ensure no data left. But this will cause tthe_tuner
	 * hangs until next print into it. This workaound is to record the hit
	 * and let tthe_tuner not pending at the next print(by 'return 0').
	 */
	if ((ret == fifo_len) && (ret == count))
		list->boundary_hit = true;
		...
		u8 *read_buf, u16 read_buf_size, u16 *crc)
{
	...
	/*
	 * According to 001-30009*I: Length of Payload = (ARL+9).
	 */
	extra_bytes = 9;
...
{
	int rc = 0;
	/* Write Offset + write_len + ebid */
	int full_write_length = 2 + 2 + 1 + write_length;
	u8 *full_write_buf;
	u8 cmd_offset = 0;
	u8 *data;
	struct pt_pip3_cmd pip3_cmd;
...
/*******************************************************************************
 * FUNCTION: pt_pip3_switch_image_
 *
 * SUMMARY: Send PIP3 SWITCH_IAMGE command 0x04 to the DUT
 *
 *	NOTE: The DUT must be in BL mode for this command to work
 ...
 * PARAMETERS:
 *	*cd       - pointer to core data
 *	 image_id - Image id sent with SWITCH_IMAGE command
 ******************************************************************************/
int pt_pip3_switch_image_(struct pt_core_data *cd, u8 image_id)
{
	int rc = 0;
	u8  data[2];
	struct pt_pip3_cmd pip3_cmd;
...
/*******************************************************************************
 * FUNCTION: pt_pip3_switch_img_store
 *
 * SUMMARY: Store method for the switch_img sysfs node. Stores what image
 *	ID to send with the SWITCH_IMAGE command in show method.
 *
 ...
 *	 size - size of buf
 ******************************************************************************/
static ssize_t pt_pip3_switch_img_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	mutex_lock(&cd->system_lock);
	pt_debug(dev, DL_INFO, "%s: Input Image ID: %d\n", __func__,
		input_data[0]);
	if (input_data[0] >= 0 && input_data[0] <= 2)
		cd->img_id = input_data[0];
	else {
		pt_debug(dev, DL_WARN, "%s: Invalid Image ID\n", __func__);
		rc = -EINVAL;
	}
	mutex_unlock(&(cd->system_lock));
...
/*******************************************************************************
 * FUNCTION: pt_pip3_switch_img_show
 *
 * SUMMARY: Show method for the switch_img sysfs node that will switch image
 *	and show the success or failure of switching image.
 *
 * NOTE: This sysfs node is only available after a DUT has been enumerated
 *
 * RETURN:
 ...
 *	*buf  - pointer to output buffer
 ******************************************************************************/
static ssize_t pt_pip3_switch_img_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct  pt_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret = 0;
	u8 mode = PT_MODE_UNKNOWN;
	u8 fw_cat_id = PT_FW_UNKNOWN;
	int status;
	u8 status_str[PT_STATUS_STR_LEN];

	if (cd->protocol_mode != PT_PROTOCOL_MODE_HID) {
		pt_debug(dev, DL_WARN,
			"%s: Protocol mode (%d) does not support this node\n",
			__func__, cd->protocol_mode);
		status = -EINVAL;
		strcpy(status_str, "Protocol mode is not HID");
		goto exit;
	}

	status = pt_pip2_get_mode_sysmode_(cd, &mode, NULL);
	if (!status && cd->mode != mode) {
		pt_debug(dev, DL_WARN,
		    "%s: cd->mode(%d) doesn't match detected(%d)\n",
		    __func__, cd->mode, mode);
		cd->mode = mode;
	...
	pt_debug(dev, DL_INFO, "%s: Current mode: %d\n", __func__,
		cd->mode);

	if (mode == PT_MODE_OPERATIONAL) {
		status = pt_pip3_get_fw_cat_id_(cd, &fw_cat_id);
		if (status) {
			strcpy(status_str,
				"Failed to get FW category ID in operational mode");
			goto exit;
		}
	}
	...
	case PT_IMG_PRIMARY:
		if (cd->mode == PT_MODE_OPERATIONAL) {
			if (fw_cat_id == PT_TOUCH_FW)
				strcpy(status_str, "Already in Primary image");
			else if (fw_cat_id == PT_PROGRAMMER_FW ||
				fw_cat_id == PT_UTILITY_FW) {
				...
				if (status) {
					strcpy(status_str,
						"SWITCH_IMAGE passed but FW category ID indicates a failure in entering primary image");
					goto exit;
				}

				if (fw_cat_id == PT_IMG_PRIMARY) {
					strcpy(status_str,
			...
			if (!status) {
				if (mode == PT_MODE_OPERATIONAL) {
					status = pt_pip3_get_fw_cat_id_(cd,
								&fw_cat_id);
					if (status) {
						strcpy(status_str,
							"FW category ID indicates a failure in entering primary image");
						goto exit;
					}
					if (fw_cat_id == PT_IMG_PRIMARY)
						strcpy(status_str,
	...
	case PT_IMG_SECONDARY:
		if (cd->mode == PT_MODE_OPERATIONAL) {
			if (fw_cat_id == PT_PROGRAMMER_FW)
				strcpy(status_str,
					"Already in Secondary image");
			else if (fw_cat_id == PT_TOUCH_FW ||
					fw_cat_id == PT_UTILITY_FW) {
				status = pt_pip3_switch_image_(cd, cd->img_id);
				if (status) {
					strcpy(status_str,
				...
				if (status) {
					strcpy(status_str,
						"SWITCH_IMAGE passed but FW category ID indicates a failure in entering secondary image");
					goto exit;
				}

				if (fw_cat_id == PT_IMG_SECONDARY) {
					strcpy(status_str,
	...
	case PT_IMG_ROM_BL:
		if (cd->mode == PT_MODE_BOOTLOADER) {
			strcpy(status_str, "Already in ROM-BL mode");
		} else {
			status = pt_pip3_switch_image_(cd, cd->img_id);
			if (!status)
...
exit:
	if (!status &&
		...
		(cd->img_id == PT_IMG_PRIMARY ||
		cd->img_id == PT_IMG_SECONDARY)) {
		pt_queue_enum(cd);
		status = _pt_request_wait_for_enum_state(
...
queue_startup:
	if (rc) {
		...
		cd->hw_detected = true;
		if (cd->enum_status <= (ENUM_STATUS_FW_RESET_SENTINEL |
		    ENUM_STATUS_BL_RESET_SENTINEL)) {
			pt_debug(cd->dev, DL_WARN,
	...
	struct device_attribute *attr, char *buf)
{
	...
	while ((cd->startup_state != STARTUP_DONE) && time < 2000) {
		msleep(50);
		pt_debug(cd->dev, DL_INFO,
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
...
#ifdef TTDL_PTVIRTDUT_SUPPORT
	if (length >= 3 && input_data[0] == 0xFF && input_data[1] == 0xFF)
		is_ctrl_cmd = true;
#endif
	...
	if (length <= 6 && input_data[0] == 0x05 && input_data[1] == 0x00) {
		opcode = input_data[3] & 0x0f;
		if (opcode == HID_CMD_RESET) {
			mutex_lock(&cd->sysfs_lock);
			cd->hid_reset_cmd_state = 1;
			mutex_unlock(&cd->sysfs_lock);
			goto uer_cmd;
		} else if (opcode == HID_CMD_GET_REPORT) {
			pm_runtime_get_sync(dev);
			rc = pt_hid_cmd_get_report(cd, input_data[2]);
			mutex_lock(&cd->sysfs_lock);
			if (!rc) {
				cd->cmd_rsp_buf_len =
				    get_unaligned_le16(&cd->response_buf[0]);
			...
			pm_runtime_put(dev);
			goto pt_command_store_exit;
		} else if (opcode == HID_CMD_SET_POWER) {
			pm_runtime_get_sync(dev);
			rc = pt_hid_cmd_set_power(cd, input_data[2]);
			mutex_lock(&cd->sysfs_lock);
			if (!rc) {
				if (cd->protocol_mode == PT_PROTOCOL_MODE_PIP) {
					cd->cmd_rsp_buf_len =
	...
	if (cd->protocol_mode == PT_PROTOCOL_MODE_HID
		...
		&& !is_ctrl_cmd) {
		pm_runtime_get_sync(dev);
		len_field = get_unaligned_le16(&input_data[1]);
		cmd_id = input_data[4] & 0x7F;
		/* For pure hid cmd if length does not include crc, add it */
		if ((len_field - 2) == (length - 1)) {
			crc = crc_ccitt_calculate(&input_data[1],
				length - 1);
		...
		cd->report_type = PIP3_CMD_REPORT;
		/* Send hid command */
		rc = pt_hid_user_cmd_send(cd, input_data, length);
		if (!rc) {
			cd->cmd_rsp_buf_len =
			    get_unaligned_le16(&cd->response_buf[0]);
			if (cd->cmd_rsp_buf_len == 0)
				cd->cmd_rsp_buf_len = 2;
			memcpy(cd->cmd_rsp_buf, cd->response_buf,
			       cd->cmd_rsp_buf_len);
		}
		...
		if (cmd_id == PIP3_CMD_ID_START_BOOTLOADER
			...
			if (rc)
				pt_debug(dev, DL_WARN, "%s: enter Gen6 BL failed\n",
					__func__);
		}
		cd->raw_cmd_status = rc;
		cd->report_type = UNKNOWN_REPORT;
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = sprintf(buf,
		...
		PT_DRV_DBG_CORE_PLATFORM_FLAG, "[0-255]", "Core Platform Flags",
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	case PT_DRV_DBG_CORE_PLATFORM_FLAG:		/* 220 */
		...
		mutex_unlock(&cd->system_lock);
		break;
	case PT_DRV_DBG_CORE_SET_PROTOCOL_MODE:		/* 221 */
		if (input_data[1] >= 0 && input_data[1] <= 2) {
			mutex_lock(&cd->system_lock);
			cd->protocol_mode = input_data[1];
			cd->set_protocol_mode = true;
			mutex_unlock(&cd->system_lock);
			pt_debug(dev, DL_INFO,
				"%s: Force protocol_mode to: 0x%02X\n",
				__func__, input_data[1]);
		} else if (input_data[1] == PT_PROTOCOL_MODE_UNKNOWN) {
			mutex_lock(&cd->system_lock);
			cd->set_protocol_mode = false;
			cd->protocol_mode = PT_PROTOCOL_MODE_UNKNOWN;
			mutex_unlock(&cd->system_lock);
			rc = pt_pip2_get_status_(cd);
			if (rc && rc != PIP2_RSP_ERR_INIT_FAILURE) {
				pt_debug(dev, DL_INFO,
					"%s: fail to detect protocol mode, try HID mode\n",
					__func__);
				cd->protocol_mode = PT_PROTOCOL_MODE_HID;
				rc = pt_pip2_get_status_(cd);
				if (rc && rc != PIP2_RSP_ERR_INIT_FAILURE) {
					cd->protocol_mode =
						PT_PROTOCOL_MODE_UNKNOWN;
					pt_debug(dev, DL_INFO,
						"%s: fail to detect protocol mode\n",
						__func__);
				}
			}
			pt_debug(dev, DL_INFO,
				"%s: Detect protocol mode: %d\n",
				__func__, cd->protocol_mode);
		} else {
			rc = -EINVAL;
			pt_debug(dev, DL_INFO,
				"%s: Invalid protocol mode: %d\n",
				__func__, input_data[1]);
		}
		break;
	case PT_DRV_DBG_CORE_SET_XRES_LEVEL:		/* 222 */
		...
		break;
	case PT_DRV_DBG_CORE_ENTER_BL_METHOD:		/* 223 */
		if (input_data[1] == PT_ENTER_BL_BY_RESET_PIN) {
			cd->enter_bl_method = PT_ENTER_BL_BY_RESET_PIN;
			pt_debug(dev, DL_INFO,
				 "%s: Enter BL by toggling reset pin\n",
				 __func__);
		} else if (input_data[1] == PT_ENTER_BL_BY_COMMAND) {
		...
		break;
	case PT_DRV_DBG_DUAL_MCU_AVAILABLE:		/* 224 */
		if (input_data[1] == 1) {
			cd->dual_mcu_available = true;
			pt_debug(dev, DL_INFO,
				 "%s: Dual mcu available\n",
				 __func__);
	...
	case PT_DRV_DBG_VIRTUAL_I2C_DUT:		/* 299 */
		if (input_data[1] == 0) {
			...
			pt_debug(dev, DL_WARN, "%s: Canceling Work\n",
				__func__);
#ifdef PT_PTSBC_SUPPORT
		...
		struct device_attribute *attr, char *buf)
{
	...
	int t;
	int rc = 0;
	int wait_time = PT_REQUEST_ENUM_TIMEOUT;
	...
	/*
	 * Double wait_time of enumeration when DUT generation is unknown
	 * because DUT detection consumes much more time.
	 */
	if (cd->active_dut_generation == DUT_UNKNOWN)
		wait_time = PT_REQUEST_ENUM_TIMEOUT * 2;
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = sprintf(buf,
		...
		"Protocol Mode             ",
			...
			"Unknown" :
			((cd->protocol_mode == PT_PROTOCOL_MODE_HID ?
			"HID" : (cd->protocol_mode == PT_PROTOCOL_MODE_PIP ?
			"PIP" : "Hybrid HID"))),
			cd->protocol_mode == PT_PROTOCOL_MODE_UNKNOWN ?
			"" : (cd->set_protocol_mode ? "(Set)" : "(Detected)"),
		"Mode                      ",
			cd->mode ? (cd->mode == PT_MODE_OPERATIONAL ?
			"Operational" : "BL-ROM") : "Unknown",
		"FW category ID            ",
			cd->fw_category_id == PT_TOUCH_FW ?
			"Touch" :
			(cd->fw_category_id == PT_PROGRAMMER_FW ?
			"Programmer" :
			(cd->fw_category_id == PT_UTILITY_FW ?
			"Utility" : "Unknown")),
		"Flashless Mode            ",
		...
		"Sentinel Count            ", cd->sentinel_count
#endif /* TTDL_DIAGNOSTICS */
	);
		...
		struct device_attribute *attr, char *buf)
{
	...
	case PT_ENTER_BL_RESET_FAIL:
		ret = snprintf(buf, PT_MAX_PRBUF_SIZE, "Status: %d\n%s\n",
			...
			"  Soft Reset Failed");
		break;
	case PT_ENTER_BL_START_BL_CMD_FAIL:
		ret = snprintf(buf, PT_MAX_PRBUF_SIZE, "Status: %d\n%s\n",
			rc,
		...
		struct device_attribute *attr, char *buf)
{
	 ...
	 */
	if (!rc && (!(cd->enum_status & ENUM_STATUS_FW_OUT_OF_BOOT))) {
		rc = pt_enum_with_dut(cd, false, &cd->enum_status);
		if (!(cd->enum_status & ENUM_STATUS_FW_OUT_OF_BOOT)) {
			strcpy(status_str,
			       "Already in APP mode - FW stuck in Boot mode");
		}
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = sprintf(buf,
		...
		"Panel ID Support         ",
			cd->panel_id_support,
		pdata->core_pdata->panel_id_support != cd->panel_id_support ?
		"(Manual Override)" : "");
	return ret;
}
	...
	u8 *bus_toggled, u8 *irq_toggled, u8 *xres_toggled, u8 *err_str)
{
	...
	if (!rc && ((cd->enum_status & ENUM_STATUS_BL_RESET_SENTINEL) ||
	    (cd->enum_status & ENUM_STATUS_FW_RESET_SENTINEL))) {
...
exit:
	...
	if (slave->irq_err_str) {
		if (last_err && detected)
			sprintf(slave->irq_err_str, "%s 0x%02X",
			...
			strcpy(slave->irq_err_str,
				"- Likely stuck low. No Critical Boot Error");
		else if (!detected)
			strcpy(slave->irq_err_str,
				"- Likely stuck high. No Critical Boot Error.");
	}
	...
	struct pt_bist_data *slave)
{
	...
	if (rc) {
		...
		if (slave->bus_err_str)
			strcpy(slave->bus_err_str,
				"- State could not be determined.");
		goto exit;
	}

	pt_debug(dev, DL_INFO, "%s Attempt open file 0\n", __func__);
	rc = _pt_pip2_file_open(dev, PIP2_RAM_FILE);
	if (rc < 0) {
		rc = -ENOENT;
		bus_toggled = false;
		pt_debug(dev, DL_WARN,
		...
		file_handle = rc;
		bus_toggled = true;
		rc = _pt_pip2_file_close(dev, file_handle);
		if (rc) {
			pt_debug(dev, DL_ERROR,
				 "%s Failed to close file, handle=%d, r=%d\n",
				 __func__, file_handle, rc);
		}
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	idx_status = sprintf(buf, "Status: %d\n",
...
exit:
	...
	if (cd->flashless_dut) {
		 ...
		 */
		rc = _pt_request_wait_for_enum_state(
		    dev, 400, ENUM_STATUS_FW_RESET_SENTINEL);
		if ((cd->enum_status & ENUM_STATUS_FW_RESET_SENTINEL) &&
		    (cd->enum_status & ENUM_STATUS_COMPLETE) == 0) {
...
/*******************************************************************************
 * FUNCTION: pt_report_rate_store
 *
 * SUMMARY: Store method for the report_rate sysfs node that will takes a passed
 ...
 *	 size - size of buf
 ******************************************************************************/
static ssize_t pt_report_rate_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	u32 input_data[2];
...
/*******************************************************************************
 * FUNCTION: pt_report_rate_show
 *
 * SUMMARY: Show method for the report_rate sysfs node that will show the
 *	results of the T-Refresh timer counting the time it takes to see a user
 *	defined number of interrupts.
 *
 * RETURN:
 *	 0 = success
 *	!0 = failure
 ...
 *	*buf  - pointer to output buffer
 ******************************************************************************/
static ssize_t pt_report_rate_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	struct pt_core_data *cd = dev_get_drvdata(dev);
	...
	if (cd->t_refresh_count != cd->t_refresh_total) {
		...
		ret = sprintf(buf,
			...
			"Average Report Rate (ms) ", whole, fraction,
			"Min INT Period (ms)      ", cd->int_period_min/1000,
					cd->int_period_min%1000,
			"Max INT Period (ms)      ", cd->int_period_max/1000,
					cd->int_period_max%1000);
	}
	mutex_unlock(&cd->system_lock);
	return ret;
}
...
static struct device_attribute attributes[] = {
	...
	__ATTR(switch_img, 0644,
		pt_pip3_switch_img_show, pt_pip3_switch_img_store),
#ifdef EASYWAKE_TSG6
	...
	__ATTR(report_rate, 0644, pt_report_rate_show, pt_report_rate_store),
#endif /* TTDL_DIAGNOSTICS */
};
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	cd->bl_pip_ver_ready           = false;
	cd->app_pip_ver_ready          = false;
	cd->pip2_cmd_tag_seq           = 0x08; /* PIP2 TAG=1b SEQ=000b */
	cd->get_param_id               = 0;
	cd->watchdog_enabled           = 0;
	cd->startup_retry_count        = 0;
	...
	/* Set platform protocol_mode value */
	if (cd->cpdata->protocol_mode != PT_PROTOCOL_MODE_UNKNOWN) {
		cd->protocol_mode = cd->cpdata->protocol_mode;
		cd->set_protocol_mode = true;
	} else {
		cd->protocol_mode = PT_PROTOCOL_MODE_PIP;
		cd->set_protocol_mode = false;
...
retry_hw_detect:
...
#ifdef TTDL_PTVIRTDUT_SUPPORT
			/*
			 * Interrupt is generated during pt_i2c module probe
			 * which could confuse detect function, force flushing
			 * bus to clear sentinel as the interrupt pin is not
			 * asserted always like a real DUT.
			 */
			pt_flush_bus(cd, PT_FLUSH_BUS_BASED_ON_LEN, NULL);
#endif
			rc = _pt_detect_dut_generation(cd->dev,
	...
	if (pip_ver_major == 2) {
		...
		pt_debug(dev, DL_WARN,
			...
			pip_ver_minor);
		cd->hw_detected = true;
	} else if (pip_ver_major == 1 || pip_ver_major == 3) {
		cd->app_pip_ver_ready = true;
		pt_debug(dev, DL_WARN, " === PIP%d.%d Detected ===\n",
			 pip_ver_major, pip_ver_minor);
		cd->hw_detected = true;
	} else {
	...
	pt_debug(dev, DL_WARN, "%s: cd->enum_status=0x%04X status=0x%04X\n",
		__func__, cd->enum_status, status);
	if (rc == -ENODEV) {
		pt_debug(cd->dev, DL_ERROR,
...
skip_enum:
	...
	pt_debug(dev, DL_INFO, "%s: Probe: MT, PEN, BTN\n", __func__);
	rc = pt_mt_probe(dev);
	if (rc < 0) {
	...
	rc = pt_pen_probe(dev);
	if (rc < 0) {
		pt_debug(dev, DL_ERROR, "%s: Error, fail pen probe\n",
			__func__);
...
error_after_startup_pen:
	pr_err("%s PARADE error_after_startup_pen\n", __func__);
	pt_pen_release(dev);
...
int pt_release(struct pt_core_data *cd)
{
	...
	sysfs_remove_group(&dev->kobj, &early_attr_group);
	device_remove_bin_file(dev, &bin_attr_pt_response);
