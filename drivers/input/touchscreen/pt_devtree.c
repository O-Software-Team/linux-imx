		...
		struct device_node *core_node)
{
	...
	rc = of_property_read_u32(core_node, "parade,protocol_mode",
		&value);
	if (!rc) {
		pdata->protocol_mode = (u8)value;
	} else {
		pr_err("%s: protocol_mode is not configured, set default: PT_PROTOCOL_MODE_PIP\n",
			__func__);
